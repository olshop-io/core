...
function login($u, $p)
{
    ...
    if (mysqli_num_rows($q)) {
        $_SESSION['login'] = true;
        $_SESSION['id'] = getSingleValDB("account", "username", $u, "id");
        $_SESSION['username'] = $u;
...
{
    global $koneksi;
    $q = mysqli_query($koneksi, "INSERT INTO `$table`($fields)
            VALUES($values)");

}


function getDataByID($table, $where, $param)
{
    global $koneksi;
    $q = mysqli_query($koneksi, "SELECT * FROM `$table` WHERE `$where`='$param'");
    $row = mysqli_fetch_assoc($q);
...
function getListData($table, $where, $param)
{
    global $koneksi;
...
function removeDataByID($table, $where, $param){
    global $koneksi;
    $q = mysqli_query($koneksi, "DELETE FROM `$table` WHERE `$where`='$param'");
    $row = mysqli_fetch_assoc($q);
    return $row;

}

function removeDataAll($table)
{
    global $koneksi;
    $q = mysqli_query($koneksi, "DELETE FROM `$table`");
    $row = mysqli_fetch_assoc($q);
    return $row;
}
