...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $vars = parse_columns('pixel_event', $_POST);
    $stmt = $pdo->prepare("UPDATE pixel_event SET name=?,created_at=?,updated_at=?,deleted_at=? WHERE id=?");

    if(!$stmt->execute([ $name,$created_at,$updated_at,$deleted_at,$id  ])) {
        echo "Something went wrong. Please try again later.";
        header("location: error.php");
    } else {
        $stmt = null;
        header("location: pixel_event-read.php?id=$id");
    }
} else {
    ...
    if(isset($_GET["id"]) && !empty($_GET["id"])){
        ...
        $sql = "SELECT * FROM pixel_event WHERE id = ?";
            ...
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <a href="pixel_event-index.php" class="btn btn-secondary">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
