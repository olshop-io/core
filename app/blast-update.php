...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $vars = parse_columns('blast', $_POST);
    $stmt = $pdo->prepare("UPDATE blast SET nomor=?,pesan=?,media=?,jadwal=?,make_by=? WHERE id=?");

    if(!$stmt->execute([ $nomor,$pesan,$media,$jadwal,$make_by,$id  ])) {
        echo "Something went wrong. Please try again later.";
        header("location: error.php");
    } else {
        $stmt = null;
        header("location: blast-read.php?id=$id");
    }
} else {
    ...
    if(isset($_GET["id"]) && !empty($_GET["id"])){
        ...
        $sql = "SELECT * FROM blast WHERE id = ?";
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <div class="form-group">
                                <label>nomor</label>
                                <textarea name="nomor" class="form-control"><?php echo $nomor ; ?></textarea>
                        ...
                        <a href="blast-index.php" class="btn btn-secondary">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
