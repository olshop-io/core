...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $stmt = $pdo->prepare("UPDATE link_detail SET link_id=?,operator_id=?,op_qty=? WHERE id=?");

    if(!$stmt->execute([ $link_id,$operator_id,$op_qty,$id  ])) {
        echo "Something went wrong. Please try again later.";
        header("location: error.php");
    } else {
        $stmt = null;
        header("location: link_detail-read.php?id=$id");
    }
} else {
