...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $vars = parse_columns('domain', $_POST);
    $stmt = $pdo->prepare("UPDATE domain SET name=?,user_id=?,created_at=?,updated_at=?,deleted_at=? WHERE id=?");

    if(!$stmt->execute([ $name,$user_id,$created_at,$updated_at,$deleted_at,$id  ])) {
        echo "Something went wrong. Please try again later.";
        header("location: error.php");
    } else {
        $stmt = null;
        header("location: domain-read.php?id=$id");
    }
} else {
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <div class="form-group">
                                <label>name</label>
                                <input type="text" name="name" maxlength="512"class="form-control" value="<?php echo $name; ?>">
                        ...
                        <a href="domain-index.php" class="btn btn-secondary">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
