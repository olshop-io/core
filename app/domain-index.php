            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        <h2 class="float-left">domain Details</h2>
                        <a href="domain-create.php" class="btn btn-success float-right">Add New Record</a>
                        <a href="domain-index.php" class="btn btn-info float-right mr-2">Reset View</a>
                        <a href="index.php" class="btn btn-secondary float-right mr-2">Back</a>
                    </div>

                    <div class="form-row">
                        <form action="domain-index.php" method="get">
                        <div class="col">
                    ...
                    $total_pages_sql = "SELECT COUNT(*) FROM domain";
                    $result = mysqli_query($link,$total_pages_sql);
                    $total_rows = mysqli_fetch_array($result)[0];
                    $total_pages = ceil($total_rows / $no_of_records_per_page);

                    //Column sorting on column name
                    $orderBy = array('id', 'name', 'user_id', 'created_at', 'updated_at', 'deleted_at');
                    $order = 'id';
                    ...
                    $sql = "SELECT * FROM domain ORDER BY $order $sort LIMIT $offset, $no_of_records_per_page";
                    $count_pages = "SELECT * FROM domain";


                    if(!empty($_GET['search'])) {
                        $search = ($_GET['search']);
                        $sql = "SELECT * FROM domain
                            WHERE CONCAT_WS (id,name,user_id,created_at,updated_at,deleted_at)
                            LIKE '%$search%'
                            ORDER BY $order $sort
                        ...
                        if(mysqli_num_rows($result) > 0){
                            ...
                            echo "<table class='table table-bordered table-striped'>";
                                ...
                                while($row = mysqli_fetch_array($result)){
                                    echo "<tr>";
                                    echo "<td>" . $row['id'] . "</td>";echo "<td>" . $row['name'] . "</td>";echo "<td>" . $row['user_id'] . "</td>";echo "<td>" . $row['created_at'] . "</td>";echo "<td>" . $row['updated_at'] . "</td>";echo "<td>" . $row['deleted_at'] . "</td>";
                                        echo "<td>";
                                            echo "<a href='domain-read.php?id=". $row['id'] ."' title='View Record' data-toggle='tooltip'><i class='far fa-eye'></i></a>";
                                            echo "<a href='domain-update.php?id=". $row['id'] ."' title='Update Record' data-toggle='tooltip'><i class='far fa-edit'></i></a>";
                                            echo "<a href='domain-delete.php?id=". $row['id'] ."' title='Delete Record' data-toggle='tooltip'><i class='far fa-trash-alt'></i></a>";
                                        echo "</td>";
                                    echo "</tr>";
                                }
                                echo "</tbody>";
