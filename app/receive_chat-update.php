...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $stmt = $pdo->prepare("UPDATE receive_chat SET id_pesan=?,nomor=?,pesan=?,from_me=?,nomor_saya=?,tanggal=? WHERE id=?");

    if(!$stmt->execute([ $id_pesan,$nomor,$pesan,$from_me,$nomor_saya,$tanggal,$id  ])) {
        echo "Something went wrong. Please try again later.";
        header("location: error.php");
    } else {
        $stmt = null;
        header("location: receive_chat-read.php?id=$id");
    }
} else {
